...
# arm sliders
topslider_center = (640, 230)
botslider_center = (640, 310)
slider_length = 150
...
# buttons
right_button_center = (255,255)
left_button_center = (210,300)
button_radius = 27.5
...
def draw_arm(screen, base, angle2, angle1):
    armbot = 50
    armtop = 50
    armwidth = 20
    #topoffset = math.radians(-150)
    topoffset = -2.556839856565194
    #botoffset = math.radians(-60)
    botoffset = -1.0271229114213778

    # bottom arm
    dir1 = pygame.Vector2(0, -armbot).rotate_rad(angle1 + botoffset)
    joint = pygame.Vector2(base) + dir1

    perp1 = dir1.normalize().rotate(90) * (armwidth / 2)

    pygame.draw.polygon(screen, button_color, [
        pygame.Vector2(base) + perp1,
        pygame.Vector2(base) - perp1,
        joint - perp1,
        joint + perp1,
    ])
    ...
    # top arm
    total_angle = angle1 - angle2 + botoffset - topoffset
    dir2 = pygame.Vector2(0, -armtop).rotate_rad(total_angle)
    end = joint + dir2

    perp2 = dir2.normalize().rotate(90) * (armwidth / 2)

    pygame.draw.polygon(screen, button_pressed_color, [
        joint + perp2,
        joint - perp2,
        end - perp2,
        end + perp2,
    ...
    # draw joints + label
    pygame.draw.circle(screen, (225,225,225), (int(base[0]), int(base[1])), 10)
    pygame.draw.circle(screen, (225,225,225), (int(joint.x), int(joint.y)), 10)
    pygame.draw.circle(screen, (225,225,225), (int(end.x), int(end.y)), 10)
    pygame.draw.circle(screen, (100,100,100), (int(base[0]), int(base[1])), 8)
    pygame.draw.circle(screen, (100,100,100), (int(joint.x), int(joint.y)), 8)
    pygame.draw.circle(screen, (100,100,100), (int(end.x), int(end.y)), 8)
    screen.blit(font.render("Arm Model", True, (225,225,225)), (base[0]-60, min(int(end.y)-40,base[1]-100)))
...
try:
    while running:
        ...
        for x in range(0, screen_dimensions[0], 25):
            pygame.draw.line(screen, (38,38,43), (x,0), (x,screen_dimensions[1]))
        for y in range(0, screen_dimensions[1], 25):
            pygame.draw.line(screen, (38,38,43), (0,y), (screen_dimensions[0],y))
        ...
        draw_arm(screen, arm_base, angle2=angletop, angle1=anglebot)

        # sliders
        pygame.draw.line(screen, (100,100,100), (topslider_center[0]-slider_length//2, topslider_center[1]), (topslider_center[0]+slider_length//2, topslider_center[1]),20)
        pygame.draw.circle(screen, (100,100,100), (topslider_center[0]-slider_length//2, topslider_center[1]+1), 10)
        pygame.draw.circle(screen, (100,100,100), (topslider_center[0]+slider_length//2, topslider_center[1]+1), 10)
        ...
        screen.blit(font.render(f"Top: {math.degrees(angletop):.1f}°", True, (225,225,225)), (390, 40))
    
        pygame.draw.line(screen, (100,100,100), (botslider_center[0]-slider_length//2, botslider_center[1]), (botslider_center[0]+slider_length//2, botslider_center[1]),20)
        pygame.draw.circle(screen, (100,100,100), (botslider_center[0]-slider_length//2, botslider_center[1]+1), 10)
        pygame.draw.circle(screen, (100,100,100), (botslider_center[0]+slider_length//2, botslider_center[1]+1), 10)
        ...
        screen.blit(font.render(f"Bottom: {math.degrees(anglebot):.1f}°", True, (225,225,225)), (390, 75))
